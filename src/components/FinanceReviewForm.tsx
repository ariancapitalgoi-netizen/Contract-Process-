...
export function FinanceReviewForm({
  ...
  // Finance-specific form states
  const [isOtherExpert, setIsOtherExpert] = useState(() => localStorage.getItem('finance_isOtherExpert') || 'خیر');
  const [greenComment, setGreenComment] = useState(() => localStorage.getItem('finance_greenComment') || '');
  const [yellowComment, setYellowComment] = useState(() => localStorage.getItem('finance_yellowComment') || '');
  const [redComment, setRedComment] = useState(() => localStorage.getItem('finance_redComment') || '');
  const [decision, setDecision] = useState(() => localStorage.getItem('finance_decision') || '');
  const [attachment, setAttachment] = useState<boolean>(() => localStorage.getItem('finance_attachment') === 'true');
  const [generalComment, setGeneralComment] = useState(() => localStorage.getItem('finance_generalComment') || '');
  ...
  const [order, setOrder] = useState(() => {
    const saved = localStorage.getItem('financeForm_order');
  ...
  React.useEffect(() => {
    ...
    localStorage.setItem('finance_yellowComment', yellowComment);
    localStorage.setItem('finance_redComment', redComment);
    localStorage.setItem('finance_decision', decision);
    localStorage.setItem('finance_attachment', attachment ? 'true' : 'false');
    localStorage.setItem('finance_generalComment', generalComment);
    localStorage.setItem('finance_stakeholderName', stakeholderName);
    localStorage.setItem('finance_estimation', estimation);
  }, [isOtherExpert, greenComment, yellowComment, redComment, decision, attachment, generalComment, stakeholderName, estimation]);

  const handleOrderChange = (newOrder: string[]) => {
    setOrder(newOrder);
    localStorage.setItem('financeForm_order', JSON.stringify(newOrder));
  };
    ...
    return (
      <input
        ...
        onChange={(e) => {
          ...
          setLabels(newLabels);
          localStorage.setItem('financeForm_labels', JSON.stringify(newLabels));
        }}
  ...
  React.useEffect(() => {
    localStorage.setItem('finance_review_notes_v2', JSON.stringify(notes));
  }, [notes]);
        ...
        <div className="flex items-center gap-6 py-1">
          {['بله', 'خیر'].map((option) => (
            <label key={option} className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="finance_isOtherExpert"
                value={option}
                checked={isOtherExpert === option}
                onChange={() => setIsOtherExpert(option)}
    ...
    attachment: (
      <>
        {isBarterContract && decision === 'تایید' && (
          <div id="finance-barter-warning" className="mb-4 bg-blue-50 text-blue-800 border-r-4 border-blue-600 p-3 rounded-sm text-[11px] font-bold shadow-sm animate-fade-in flex items-center gap-2">
    ...
    generalComment: (
      ...
      >
        <textarea
          ...
          placeholder="توضیحات تکمیلی یا نهایی بررسی مالی..."
        ></textarea>
      </FieldRowTop>
    )
  };
  ...
  return (
    <div className="flex flex-col gap-4 text-gray-700 w-full max-w-7xl mx-auto pt-4 pb-24">
      ...
      <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden animate-fade-in">
        ...
        {activeTab === 'opinion' && (
          <div className="flex flex-col lg:flex-row min-h-[500px]">
            ...
            <div className="flex-1 p-4 md:p-6 border-l border-gray-100 flex flex-col gap-5">
              ...
              {/* Feedback Textareas */}
              <div id="finance-feedback-boxes" className="flex flex-col gap-4">
                
                {/* 1. Green comment block */}
                <div id="finance-green-box" className="border border-green-500 rounded-sm overflow-hidden shadow-sm">
                ...
                <div id="finance-yellow-box" className="border border-yellow-400 rounded-sm overflow-hidden shadow-sm">
                ...
                {/* 3. Red comment block */}
                <div id="finance-red-box" className="border border-red-500 rounded-sm overflow-hidden shadow-sm">
        ...
        {activeTab === 'contractInfo' && (
          <div id="contract-info-tab" className="p-4 md:p-6 flex flex-col gap-6 text-[12px] text-gray-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-12 bg-gray-50/40 p-4 rounded border border-gray-150">
              ...
              <div id="contract-type-info-row" className="flex justify-between items-center py-1.5 border-b border-gray-100">
                ...
                <select 
                  ...
                  className="w-full max-w-[250px] border border-gray-300 rounded px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-[#b90000] text-xs"
                >
                  <option value="">- انتخاب کنید -</option>
            ...
            <div className="mt-4" id="finance-parties-table">
                ...
                <table className="w-full text-right border-collapse text-[11px]">
                  <thead>
                    <tr className="bg-gray-150 text-gray-700 border-b border-gray-300 font-bold">
                      <th className="p-2 border-l border-gray-300 min-w-[100px]"><EditableText isTestMode={isTestMode} defaultText="نوع طرف قرارداد" /></th>
                      <th className="p-2 border-l border-gray-300 min-w-[120px]"><EditableText isTestMode={isTestMode} defaultText="نام و نام خانوادگی" /></th>
                      <th className="p-2 border-l border-gray-300 min-w-[120px]"><EditableText isTestMode={isTestMode} defaultText="نام سازمان" /></th>
                      <th className="p-2 border-l border-gray-300 min-w-[100px]"><EditableText isTestMode={isTestMode} defaultText="کد ملی" /></th>
      ...
      {/* Bizagi Technical Notes for Finance Review */}
      <BizagiDevNotes 
        notes={notes} 
