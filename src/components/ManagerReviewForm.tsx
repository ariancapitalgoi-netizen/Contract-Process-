...
export function ManagerReviewForm({
  ...
  const [order, setOrder] = useState(() => {
    const saved = localStorage.getItem('reviewForm_order');
    return saved ? JSON.parse(saved) : ['decision', 'attachment', 'description'];
  });
  const [labels, setLabels] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem('reviewForm_labels');
  ...
  const handleOrderChange = (newOrder: string[]) => {
    setOrder(newOrder);
    localStorage.setItem('reviewForm_order', JSON.stringify(newOrder));
  };
    ...
    return (
      <input
        ...
        onChange={(e) => {
          ...
          setLabels(newLabels);
          localStorage.setItem('reviewForm_labels', JSON.stringify(newLabels));
        }}
  ...
  React.useEffect(() => {
    localStorage.setItem('senior_manager_review_notes', JSON.stringify(notes));
  }, [notes]);
    ...
    <div className="flex flex-col gap-4 text-gray-700 w-full max-w-4xl mx-auto xl:mr-0 pl-16 pt-4 pb-24">
      ...
      <div className="bg-white border border-gray-200 rounded-sm shadow-sm px-4 py-3 flex items-center">
        <span className="text-gray-800 text-sm">
          <EditableText isTestMode={isTestMode} defaultText="درخواست انعقاد قرارداد" /> <span className="text-gray-400 mx-1">›</span> <EditableText isTestMode={isTestMode} defaultText="بررسی درخواست توسط ..." />
        </span>
      </div>
